...
export default function HomeBubblePost({
  ...
  function likeUnlike() {
    if (user) {
      ...
      setLike(!like);
      if (newState == true) {
        likePost({ postId: post.post._id, userId: user.id });
      } else if (newState == false) {
        unlikePost({ postId: post.post._id, userId: user.id });
      }
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="w-full  flex gap-2 py-2 justify-end">
        <Button
          variant={youLikedIt == true ? "secondary" : "outline"}
          onClick={() => likeUnlike()}
        >
          <Heart fill={youLikedIt == true ? "secondary" : "outline"} />
      ...
      <Collapsible open={open} className="flex w-full flex-col gap-2">
        <CollapsibleContent className="flex flex-col gap-2">
          ...
          {getComments?.comments.map((comment, index) => (
            <div
              key={index}
              className="rounded-md border px-2 py-2 font-mono text-sm w-full shadow-md"
            >
              <div className="flex items-center gap-2 w-full">
                ...
                <div className="flex flex-col w-full">
                  <div className="flex gap-2">
                    ...
                    <Badge variant={"secondary"}>{getBubbleRoles?.role}</Badge>
                  </div>
